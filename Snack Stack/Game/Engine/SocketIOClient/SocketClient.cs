...
namespace Blok3Game.Engine.SocketIOClient
{
    public class SocketClient
    {
        ...
        public void SubscribeToDataPacket<T>(Action<T> onDataReceived) where T : DataPacket
        {
            Type type = typeof(T);
            eventHandlers[type].Add(onDataReceived);
        }
		...
		/// <typeparam name="T">A subtype of DataPacket</typeparam>
		/// <param name="onDataReceived">A function with only one parameter; the subtype provided as generic parameter</param>
        public void UnsubscribeFromDataPacket<T>(Action<T> onDataReceived) where T : DataPacket
        {
            Type type = typeof(T);
            eventHandlers[type].Remove(onDataReceived);
        }
        ...
        {
            eventHandlers.TryGetValue(dataPacket.GetType(), out List<Delegate> handlers);
			for(int i = handlers.Count - 1; i >= 0; i--)
			{
				handlers[i].DynamicInvoke(dataPacket);
			}
        }
        ...
        private void AddEventHandler<T>() where T : DataPacket
        {
            ...
            {
                eventHandlers.Add(type, new List<Delegate>());
            }
        }
